...
export const createPatient = (req: IRequestWithTokenData, res: Response, next: NextFunction) => {
  const { name, email, contact, dob, address, photoUrl, specialAttention } = req.body;
  const userId = req.id;
  if (!userId) {
...
export const getAllPatients = (req: IRequestWithTokenData, res: Response, next: NextFunction) => {
  const userId = req.id;
  if (!userId) {
    return next(new CustomError('id of user in token data is required', StatusCodes.BAD_REQUEST));
  }
...
export const updatePatient = (req: IRequestWithTokenData, res: Response, next: NextFunction) => {
  const { name, email, contact, dob, address, photoUrl, specialAttention } = req.body;
  const userId = req.id;
