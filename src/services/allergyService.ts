...
export const getAllAllergiesByPatientId = async (patientId: number): Promise<ISuccess<IAllergy[]>> => {
...
export const updateAllergy = async (allergy: IAllergy): Promise<ISuccess<IAllergy>> => {
  ...
  if (!updatedAllergy) {
    throw new CustomError("allergy  doesn't exist to edit", StatusCodes.NOT_FOUND);
  }
...
export const deleteAllergy = async (id: number): Promise<ISuccess<IAllergy>> => {
  ...
  if (!patient) {
    throw new CustomError('Couldnot delete the requested allergy', StatusCodes.NOT_FOUND);
  }
